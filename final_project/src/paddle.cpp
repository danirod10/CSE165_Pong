...
void Paddle::_ready() {
	...
	//randomly generates the balls new direction for the method _on_area_entered
	random = (Ref<RandomNumberGenerator>)RandomNumberGenerator::_new();
	random->randomize();
...
void Paddle::_on_area_entered(Ball* ptr_ball) {
	...
	if ((ptr_ball->get_name() == "Ball1") || (ptr_ball->get_name() == "Ball2")) {
		...
		Vector2 new_direction = Vector2(-(cur_direction.x), random->randf() * 2 - 1).normalized();
